...
* TODO Dancing
  ...
  #+BEGIN_SRC scheme
    ...
    (define (balance-triplets totals surprises)
        ...
        (if (null? totals)
            ...
            (let ((total (car totals)))
              (if (zero? surprises)
                  ...
                  (iter (cdr totals)
                        (- surprises 1)
                        (cons (surprising-triplet total) triplets)))))))
    
    (define (best-greater-than triplets p)
      (count (lambda (best) (>= best p))
             (map best triplets)))
    ...
    (trace mod-three-triplet)
    (trace surprising-triplet)
    (trace unsurprising-triplet)
